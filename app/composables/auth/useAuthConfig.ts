...
export const useAuthConfig = () => {
   ...
   * 获取登录配置数据
   */
  const { data: loginConfigData, pending: isConfigLoading } = useAsyncData(
    'login-config', 
  ...
  const getBrandingConfig = () => {
    return loginConfig.value.branding || {
      name: '忆滤',
      tagline: 'AI 遗忘引擎'
    }
  }
  ...
  const getLabelsConfig = () => {
    return loginConfig.value.labels || {
      showPassword: '显示密码',
      hidePassword: '隐藏密码',
      loading: '载入中...',
      backToHome: '返回首页'
  ...
  const getErrorsConfig = () => {
    return loginConfig.value.errors || {
