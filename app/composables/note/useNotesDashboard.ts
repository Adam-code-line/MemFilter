...
export const useNotesDashboard = (options: NoteDashboardOptions = {}) => {
  ...
  const initializePromise = notesStore.ensureInitialized(
  ...
  const filteredNotes = computed(() => {
    ...
    if (timeFilter.value !== "all") {
      ...
      const thresholdDays =
        filterMap[timeFilter.value as Exclude<TimeFilter, "all">];
    ...
    data.sort((a, b) => {
      ...
      const timestampDelta =
        (extractNoteTimestamp(b) ?? 0) - (extractNoteTimestamp(a) ?? 0);
      if (timestampDelta !== 0) {
