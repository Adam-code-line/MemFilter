...
<template>
  ...
  >
    <div class="flex flex-col" :class="isCompact ? 'gap-3' : 'gap-4'">
      ...
      <p v-if="!isCompact" class="text-sm leading-relaxed text-slate-200/80">
        {{ props.item.summary }}
      </p>
      <p v-else class="text-xs leading-relaxed text-slate-300/80 line-clamp-3">
        {{ props.item.summary }}
      </p>
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div
          class="flex items-center gap-2 text-slate-400/70"
          :class="isCompact ? 'text-[11px]' : 'text-xs'"
        >
          <UIcon
            :name="isCompact ? 'i-lucide-notebook-pen' : 'i-lucide-lightbulb'"
            class="text-sky-300"
          />
