<template>
  <section
    class="flex flex-col gap-6 rounded-3xl border border-slate-200/50 bg-white/90 p-6 shadow-sm transition-opacity dark:border-slate-700/60 dark:bg-slate-900/80"
  >
    <header :class="['flex flex-col gap-6', fadeLevel > 0 ? 'opacity-90' : '']">
      <div
      ...
      >
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div class="flex min-w-0 flex-1 flex-col gap-2">
            <h2
              v-if="headerTitle"
              class="break-words text-2xl font-semibold text-slate-900 dark:text-slate-100"
            >
              {{ headerTitle }}
            </h2>
            <p
      ...
      <div
        class="flex flex-col gap-5 rounded-2xl border border-slate-200/50 bg-white/95 p-5 dark:border-slate-700/60 dark:bg-slate-900/70"
      >
        <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-wrap items-center gap-3">
            <USelectMenu
              ...
              :ui="{ menu: 'min-w-[160px]' }"
            />
            <UBadge v-if="saveStatus" :label="saveStatus" :color="statusColor" variant="outline" />
          </div>
          ...
          <UTextarea
            ...
            :ui="{
              wrapper: 'w-full',
              textarea:
                'w-full resize-none rounded-xl border border-slate-200/60 bg-white/95 px-4 py-3 text-base leading-relaxed text-slate-800 focus:border-primary focus:ring-1 focus:ring-primary/40 dark:border-slate-700/60 dark:bg-slate-900/60 dark:text-slate-100',
            }"
          />
        </div>
        <div class="flex flex-col gap-2">
          ...
          <UTextarea
            ...
            :ui="{
              wrapper: 'w-full',
              textarea:
                'w-full rounded-xl border border-slate-200/60 bg-white/95 px-4 py-3 text-sm leading-relaxed text-slate-700 focus:border-primary focus:ring-1 focus:ring-primary/40 dark:border-slate-700/60 dark:bg-slate-900/60 dark:text-slate-200',
            }"
          />
        </div>
      </div>
    ...
    <footer
      class="flex flex-wrap items-center justify-between gap-4 text-sm text-slate-600 dark:text-slate-300"
    >
      ...
      <div class="flex gap-3">
        <UButton variant="ghost" size="sm" @click="emit('cancel')">
          {{ config.actions?.cancel ?? '取消' }}
        </UButton>
        <UButton color="primary" size="sm" :loading="isSaving" @click="handleSave">
...
<script setup lang="ts">
  ...
  const statusColor = computed(() => (saveStatus === statusLabels.saved ? 'success' : 'warning'))
  ...
  watch(
    ...
    ({ mode, title, content, fade, importance, description, evaluation, compression }) => {
      const hasEditPayload =
        title !== undefined || content !== undefined || description !== undefined
      ...
      if (mode === 'edit' && hasEditPayload) {
        ...
        saveStatus.value =
          noteContent.value || noteDescription.value ? statusLabels.saved : statusLabels.unsaved
