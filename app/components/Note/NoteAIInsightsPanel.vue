<template>
  <section class="flex flex-col gap-5 rounded-2xl border border-slate-200/60 bg-slate-50/80 p-5 shadow-sm dark:border-slate-700/60 dark:bg-slate-900/40">
    <div class="flex flex-col gap-4 rounded-xl border border-slate-200/60 bg-white/95 p-4 dark:border-slate-700/60 dark:bg-slate-900/70">
      <header class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex flex-col">
          <span class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">AI 价值评估</span>
          <span class="text-sm text-slate-500 dark:text-slate-300">自动判断重要程度并给出处理建议</span>
        </div>
      ...
      <div v-if="evaluation" class="flex flex-col gap-4">
        ...
        <div class="flex flex-wrap gap-3 text-xs text-slate-500 dark:text-slate-400">
          <span v-if="evaluation.usage?.promptTokens">Prompt：{{ evaluation.usage.promptTokens }}</span>
          <span v-if="evaluation.usage?.completionTokens">Completion：{{ evaluation.usage.completionTokens }}</span>
          <span v-if="evaluation.usage?.totalTokens">总计：{{ evaluation.usage.totalTokens }}</span>
        </div>
      </div>
      ...
      <footer class="flex flex-wrap items-center gap-3 pt-2">
        <UButton
          size="sm"
          color="primary"
          :loading="evaluating"
        ...
        <span v-if="!contentReady" class="text-xs text-slate-500 dark:text-slate-400">需要更多正文内容后才能调用评估。</span>
      </footer>
    </div>
    ...
    <div class="flex flex-col gap-4 rounded-xl border border-slate-200/60 bg-white/95 p-4 dark:border-slate-700/60 dark:bg-slate-900/70">
      <header class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex flex-col">
          <span class="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">AI 摘要压缩</span>
      ...
      <footer class="flex flex-wrap items-center gap-3 pt-2">
        ...
        <span v-if="!contentReady" class="text-xs text-slate-500 dark:text-slate-400">输入足够正文后即可生成摘要。</span>
      </footer>
    </div>
  </section>
</template>
...
import type { ImportanceLevel, NoteAIEvaluation, NoteAICompression, NoteAISuggestedAction } from '~/composables/note/types'
...
const evaluationConfidence = computed(() =>
  evaluation.value ? Math.round(Math.min(100, Math.max(0, evaluation.value.confidence * 100))) : null
)
...
watch(() => props.evaluation, value => {
  evaluation.value = value ?? null
  evaluationStale.value = false
})

watch(() => props.compression, value => {
