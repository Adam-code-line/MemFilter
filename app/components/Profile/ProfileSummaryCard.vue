<script setup lang="ts">
    ...
    return props.stats.map((stat) => {
      ...
      const normalizedOverride =
        override !== undefined && override !== null && String(override).trim() !== ''
          ...
          : undefined
      const fallbackValue =
        typeof stat.value === 'string' && stat.value.trim().length > 0 ? stat.value : '--'
  ...
  const displayBio = computed(
    () => safeSummary.value.bio?.trim() || '添加一段简介，让团队更了解你的记忆偏好。'
  )
        ...
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1 space-y-2">
            <div class="flex flex-wrap items-center gap-2">
              ...
              />
              <UBadge v-else label="状态待设置" color="neutral" variant="soft" />
            </div>
          ...
          <div class="flex items-start justify-between gap-3">
            <div class="space-y-1">
              ...
              <p class="text-2xl font-semibold text-gray-900 dark:text-white">
                {{ stat.value
                }}<span v-if="stat.unit" class="ml-1 text-base text-gray-500">{{ stat.unit }}</span>
              </p>
            ...
            <div class="flex flex-col items-end gap-1">
              ...
              >
                <UIcon :name="resolveTrendIcon(stat.trend?.direction)" class="text-sm" />
                {{ stat.trend.value }}
              </span>
            </div>
          </div>
