...
import { useAIChatSessions, type ChatSessionRecord } from '~/composables/chat/useAIChatSessions'
...
const currentSessionTitle = computed(() => currentSession.value?.title ?? '新的对话')
...
const loadSession = (session: ChatSessionRecord | null) => {
  ...
  if (session) {
    replaceMessages(session.messages?.length ? session.messages : getInitialMessages())
  ...
  <div class="relative min-h-screen bg-slate-950 text-white">
    ...
    <UContainer class="relative flex min-h-screen flex-col gap-10 py-12">
      ...
      <div class="flex w-full flex-1 flex-col gap-6 lg:flex-row">
        ...
        >
          <template #header>
            <div class="flex flex-wrap items-start justify-between gap-4 text-sm text-white/70">
              <div class="space-y-2">
                <div class="flex items-center gap-2 text-xs uppercase tracking-[0.3em] text-white/50">
                  <UIcon name="i-lucide-message-circle" class="h-4 w-4 text-primary" />
                  <span>即时对话</span>
                </div>
                <div>
                  <h2 class="text-xl font-semibold text-white">{{ currentSessionTitle }}</h2>
