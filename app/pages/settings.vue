<script setup lang="ts">
  ...
  const resolveSaveTarget = (cardKey: string): SaveTarget =>
    cardSaveTargetMap[cardKey] ?? 'preferences'
  ...
  const resolveFooterHint = (cardKey: string) =>
    cardFooterHintMap[cardKey] ?? '更改后记得保存设置。'
...
<template>
  <div class="max-w-6xl mx-auto space-y-12 pb-20">
    ...
    <section v-for="section in allSections" :key="section.key" class="space-y-6">
