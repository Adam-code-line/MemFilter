...
const timeLabel = computed(
  () => searchConfig.value.timeLabel ?? noteDefaults.search.timeLabel
);
...
const isEditingExisting = computed(
  () => editorMode.value === "edit" && !!editingNote.value
);

const editingBadge = computed(() =>
  editingNote.value
    ? resolveImportanceBadge(editingNote.value.importance)
    : null
);
...
const listConfigResolved = computed(
  () => listConfig.value ?? noteDefaults.list
);
const noteListHeader = computed(
  () => listConfigResolved.value.title ?? noteDefaults.list.title
);
...
const listHeaderIcon = computed(
  () =>
    listConfigResolved.value.icon ??
    noteDefaults.list.icon ??
    "i-lucide-notebook-pen"
);

const mergedEmptyState = useEmptyState(
  emptyStateConfig,
  noteDefaults.emptyState
);
...
const totalNotesBadge = computed(
  () => `${totalNotesLabel.value}: ${notes.value.length}`
);
...
const handleTimeFilterChange = (value: string | null) => {
  ...
  const nextValue: TimeFilterValue = timeFilterValues.includes(
...
const handleEditorEvaluationUpdated = async (
...
const handleEditorCompressionUpdated = async (
  ...
  <div class="max-w-7xl mx-auto space-y-8 px-4 sm:px-6 lg:px-8">
    ...
    >
      <div class="space-y-3">
        ...
        <p
          v-if="headerSubtitle"
          class="text-gray-600 dark:text-gray-400 max-w-2xl"
        >
          {{ headerSubtitle }}
        </p>
      </div>
