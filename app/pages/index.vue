<script setup lang="ts">
  ...
  useHead({
    ...
    meta: [
      {
        name: 'description',
        content: '用 AI 主动帮助用户遗忘不重要的信息，保留关键内容。信息去噪，智能保留。',
      },
    ],
  })
  ...
  const sanitizeLink = (link: Record<string, unknown>) => {
    ...
    const sanitizedIcon = typeof icon === 'string' ? sanitizeIcon(icon) : undefined
    return {
      ...rest,
      label: typeof label === 'string' && label.length > 0 ? label : '',
      to: typeof to === 'string' && to.length > 0 ? to : typeof action === 'string' ? action : '',
...
<template>
  <div v-if="page">
    ...
    <FloatingSection :title="features.title" :description="features.description" :delay="2">
