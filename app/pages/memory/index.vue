...
const sortByImportance = (collection: NoteRecord[]) =>
  [...collection].sort((a, b) => {
    const importanceDelta =
      (importancePriority[a.importance as keyof typeof importancePriority] ??
        99) -
      (importancePriority[b.importance as keyof typeof importancePriority] ??
        99);
...
const resolveMemoryBucket = (
  note: NoteRecord
): "fresh" | "fading" | "archived" => {
...
const sections = computed(() =>
  sectionSource.value.map((section) => {
    const defaults = memoryDefaults.sections.find(
    ...
    return {
      ...
      empty: {
        title:
          section.empty?.title ??
          emptyDefaults.title ??
          detail.value.emptyFallback.title ??
          "",
        description:
  ...
  <div class="max-w-6xl mx-auto space-y-10">
    <section class="space-y-4">
      <div class="flex flex-col gap-3">
        ...
        <p
          v-if="headerSubtitle"
          class="text-sm text-gray-500 dark:text-gray-400 max-w-3xl"
        >
          {{ headerSubtitle }}
        </p>
        <p
          v-if="introDescription"
          class="text-gray-600 dark:text-gray-400 max-w-3xl"
        >
          {{ introDescription }}
        </p>
      </div>
      ...
      >
        <div v-if="isClientReady" class="grid gap-6 sm:grid-cols-3">
          ...
          >
            ...
            <span :class="['text-2xl font-semibold', stat.colorClass]">{{
              stat.value
            }}</span>
      ...
      <UCard class="border border-gray-200/70 dark:border-white/10">
        ...
        <div v-if="isClientReady">
          ...
          <UAlert
            ...
            :title="
              section.empty.title || detail.emptyFallback.title || '暂无内容'
            "
            :description="
              section.empty.description ||
              detail.emptyFallback.description ||
              '稍后再来看看。'
            "
            color="neutral"
            variant="soft"
