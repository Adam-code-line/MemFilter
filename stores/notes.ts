...
export const useNotesStore = defineStore('notes', () => {
  ...
  const sortedByRecency = computed(() =>
    notes.value.slice().sort((a, b) => resolveNoteTimestamp(b) - resolveNoteTimestamp(a))
  )

  const sortedByImportance = computed(() =>
    notes.value.slice().sort((a, b) => {
      const importanceDelta =
        (importancePriority[a.importance] ?? 99) - (importancePriority[b.importance] ?? 99)
  ...
  const upsertNote = async (payload: NoteSavePayload, existing?: NoteRecord | null) => {
    ...
    if (existing) {
      ...
      const nextEvaluation =
        payload.aiEvaluation === undefined ? (previous.aiEvaluation ?? null) : payload.aiEvaluation
